...
#define DEGREES_TO_RADIANS (M_PI / 180.0f)
	...
	struct Vec2 {
		...
		inline const float operator [] (int index) const {  /// This one is for reading the Vec2 as if where an array
			return *(&x + index);
		}

		inline float& operator [] (int index) {	/// This one is for writing to the Vec2 as if where an array.  
		...
		/// Multiply a scaler by a Vec2  - Ha! It's the scalar first then the Vec2
